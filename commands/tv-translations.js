...
module.exports = {
	...
	async execute(interaction) {
		...
		if (!tvTitles.length) {
			await interaction.reply({ embeds: [createNoResultEmbed(Colors.Red, 'No TV Shows Found for that query', 'Please make a new command with a different options')], files:[file] });
			return;
		}
		...
		const embed = createEmbed(Colors.Blue, 'TV Show Translations will appear here', 'Some description here', 'https://discord.js.org/');
		...
		selectMenucollector.on(MyEvents.Collect, async i => {
			...
			currentIndex = 0;
			const appendToRespnse = ['translations'];
