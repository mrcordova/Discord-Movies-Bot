...
module.exports = {
	...
	async execute(interaction) {
		...
		if (!tvTitles.length) {
			await interaction.reply({ embeds: [createNoResultEmbed(Colors.Red, 'No TV Shows episode Found', 'Please make a new command with a different info.')], files: [file] });
			return;
		}
		...
		const embed = createEmbed(Colors.Blue, 'TV Show episode Videos will appear here', 'Some description here', 'https://discord.js.org/');
		...
		selectMenucollector.on(MyEvents.Collect, async m => {
			...
			let tvResponse;
			try {
				tvResponse = await axios.get(`${api_url}/tv/${selected}/season/${seasonNum}/episode/${episodeNum}?api_key=${MOVIE_API_KEY}&language=${language}&append_to_response=videos&include_video_language=${vidLang},null`);
			}
			catch {
				await m.update({
