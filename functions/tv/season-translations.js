...
module.exports = {
	...
	// 	.setName('tv-season-translations')
	...
	async execute(interaction) {
		...
		if (!tvTitles.length) {
			await interaction.reply({ embeds: [createNoResultEmbed(Colors.Red, 'No TV Shows Season Found for that query', 'Please make a new command with a different options')], files:[file] });
			return;
		}
		...
		const embed = createEmbed(Colors.Blue, 'TV Show Season Translations will appear here', 'Some description here', 'https://discord.js.org/');
		...
		selectMenucollector.on(MyEvents.Collect, async i => {
			...
			let tvResponse;
			try {
				tvResponse = await axios.get(`${api_url}/${TV}/${selected}/season/${seasonNum}?api_key=${MOVIE_API_KEY}&language=${language}&append_to_response=translations`);
			}
			catch {
				await i.update({
